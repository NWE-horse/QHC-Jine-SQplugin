...
class RconConnection:
    def __init__(self):
        ...
        self.types = {'auth': 3, 'command': 2, 'response': 0, 'server': 1}
...
class re_str:
    def chat(self,data):
        matchChat  = re.match(r'\[(ChatAll|ChatTeam|ChatSquad|ChatAdmin)] \[Online IDs:EOS: ([0-9a-f]{32}) steam: (\d{17})] (.+?) : (.*)',data)
        if matchChat:
            CHAT_MESSAGE={
                "chat": matchChat[1],
                "eosID": matchChat[2],
    ...
    def SqCreated(self,data):
        matchsqCreated = re.match(r"(.+) \(Online IDs: EOS: ([\da-f]{32})(?: steam: (\d{17}))?\) has created Squad (\d+) \(Squad Name: (.+)\) on (.+)", data)
        if matchsqCreated:
            CREATE_MESSAGE={
    ...
    def plList(self,data):
        matchplrList = re.findall(r'ID: (\d*) \| Online IDs: EOS: ([\da-f]{32})(?: steam: (\d{17}))? \| Name: (.*?) \| Team ID: (\d) \| Squad ID: (.+) \| Is Leader: (.+) \| Role: (.+)',data)
        l = []
        # print(data)
        if matchplrList != []:
...
class Base:
    ...
    def request_steam(self,id):
        try:
            playtime_response = requests.get(url=f'http://api.steampowered.com/IPlayerService/GetRecentlyPlayedGames/v1/?key=1B9BD1175C06924BFF4D3BABB5115407&steamid={id}').json()['response']
            personaname = requests.get(url=f'http://api.steampowered.com/ISteamUser/GetPlayerSummaries/v2/?key=1B9BD1175C06924BFF4D3BABB5115407&steamids={id}').json()['response']['players'][0]['personaname']
            steam_level = requests.get(url=f'http://api.steampowered.com/IPlayerService/GetSteamLevel/v1/?key=1B9BD1175C06924BFF4D3BABB5115407&steamid={id}').json()['response']['player_level']
                            ...
                            if self.UndisclosedPlyaerJoinCheckWarn == True:
                                if player[1] is None:
                                    ...
                                    if self.UndisclosedPlyaerJoinHandle == True:
                                        ...
                                        threading.Thread(target=rcon.send, args=(f'AdminKick {l["steamid"]} 服务器禁止资料异常玩家加入，自动踢出',)).start()
                                    else:
                                        threading.Thread(target=rcon.send, args=(
                                            f'AdminBroadcast 玩家加入检查 | 异常玩家警告 | {player[0]}',)).start()
                                else:
                                    print('玩家加入检查',player,'类型','正常')
                                    threading.Thread(target=rcon.send, args=(f'AdminBroadcast 玩家加入检查 | {player[0]} 游戏时长: {player[2]}小时 Steam等级: {player[1]}级',)).start()
                            else:
                                if player[1] is None:
                                    if self.UndisclosedPlyaerJoinHandle == True:
                                        print('玩家加入检查', player, '类型', '异常')
...
class moudel:
    ...
    def ggl(self,chat_json,rcon):
        ...
        if sign_data != 0 and sign_data[1] != 0:
            if int(sign_data[1]) < self.PrizeDrawPoint:
                threading.Thread(target=rcon.send, args=(f'AdminBroadcast {name} 您的{self.PointName}不足，嘎嘎哭',)).start()
            else:
                raw = int(sign_data[1]) - self.PrizeDrawPoint
                nb = raw + random_number
                threading.Thread(target=rcon.send, args=(f'AdminBroadcast 嘎噶噶，{name} 本次消耗{self.PrizeDrawPoint}{self.PointName}，获得 {random_number}{self.PointName}，剩余{self.PointName}: {nb}',)).start()
                self.base.updatePoints(steamID,nb)
        elif sign_data[1] == 0:
            threading.Thread(target=rcon.send, args=(f'AdminBroadcast {name} 你还未拥有{self.PointName}，请先签到后再噶',)).start()
        ...
        if hours:
            ...
            if sign_data != 0 and sign_data[1] != 0 :
                if sign_data[1] < self.GameHoursGetNum:
                    threading.Thread(target=rcon.send,args=(f'AdminBroadcast {name} {self.PointName}不足，无法使用游戏时长查询功能',)).start()
                else:

                    number = sign_data[1] - self.GameHoursGetNum
                    if res != None:
                        rcon.send(f'AdminBroadcast {name} Squad游玩时长 {res} 小时，查询消耗 {self.GameHoursGetNum}{self.PointName}，剩余:{number}{self.PointName}')
                    else:
                        rcon.send(f'AdminBroadcast {name} 未公开资料，查询消耗 {self.GameHoursGetNum}{self.PointName}，剩余:{number}{self.PointName}')
        ...
        if hour:
            ...
            if hours == None:
                if self.UndisclosedInformationHandle == True:
                    threading.Thread(target=rcon.send,args=(f'AdminWarn {steamid} 资料未公开，服务器禁止未公开玩家建队，自动移除小队',)).start()
                    threading.Thread(target=rcon.send,args=(f'AdminRemovePlayerFromSquad {steamid}',)).start()
                else:
                    threading.Thread(target=rcon.send,args=(f'AdminBroadcast {t} {team} | {name} 创建{id}队 {squad} | 资料未公开',)).start()
                    threading.Thread(target=rcon.send,args=(f'AdminWarn {steamid} 请公开游戏资料，如因未公开资料造成的误封，后果请自行承担',)).start()
                ...
                if steamid == '76561198191782061': # 白名单
                    ...
                    threading.Thread(target=rcon.send, args=(f'AdminWarn {steamid} 你的游戏时长为 {hours}小时，未满足服务器最低 {self.CreateSquadMinHours} 小时建队时长，自动移除小队',)).start()
            else:
                for rule in Ruls:
                    for sqname in Ruls[rule]['name'].split(' '):
                        if sqname in squad.lower():
                            if int(Ruls[rule]['minhours']) > int(float(hours)):
                                threading.Thread(target=rcon.send, args=(f'AdminRemovePlayerFromSquad {steamid}',)).start()
                                threading.Thread(target=rcon.send, args=(f'AdminWarn {steamid} 你的游戏时长为 {hours}小时，服务器 {squad}队最低时长要求为{Ruls[rule]["minhours"]}小时，自动移除小队',)).start()
                                return 0
    ...
    def ChangeTeam(self,chat_json,rcon):
        ...
        if sign_data != 0 and sign_data[1] != 0 :
            if sign_data[1] < self.TeamChangeNum:
                threading.Thread(target=rcon.send, args=(f'AdminWarn {steamID} {self.PointName}不足，无法使用跳边功能',)).start()
            else:
                if count < self.TeamChangeNumber:
                    ...
                    if difference > self.ConstraintCount:
                        ...
                        rcon.send(f'AdminBroadcast {name} 跳边成功，消耗{self.TeamChangeNum}{self.PointName},剩余{self.PointName}: {number}')
                        self.base.write_teamchange_log(steamID)
                        self.base.updatePoints(steamID,number)
                else:
                    threading.Thread(target=rcon.send,args=(f'AdminWarn {steamID} 本局跳边次数已用尽，无法使用跳边功能',)).start()
        elif sign_data[1] == 0:
            ...
            if data:
                for i in data:
                    ...
                    if re.findall(r'(小队 \d)', name) != []:
                        if locked == 'True':
                            threading.Thread(target=rcon.send,args=(f'AdminWarn {steamid} 插件检测到您的队伍为步兵队，请立即打开对锁，否则自动解散小队。',)).start()
                        else:
                            pass
                    else:
                        for rule in Ruls:
                            for sqname in Ruls[rule]['name'].split(' '):
                                if sqname in name.lower() :
                                    if Ruls[rule]['lock'] != 'None' :
                                        threading.Thread(target=rcon.send, args=(
                                        f'AdminWarn {steamid} 插件检测到您的队伍 {name} 不支持锁队，请打开队锁，否则自动解散小队。',)).start()
    ...
    def QuIntegral(self,chat_json,rcon):
        ...
        if day != 0 :
            ...
            threading.Thread(target=rcon.send, args=(f'AdminBroadcast {name} 还未拥有{self.PointName}，请先签到获取',)).start()
    ...
    def KillWarn(self,chat_json,rcon):
        ...
        if day != 0:
            if int(rawnum) >= self.KillWarnNumber:
                ...
                threading.Thread(target=rcon.send, args=(f'AdminBroadcast {name} 消耗{self.KillWarnNumber}{self.PointName}，兑换击杀提示1小时成功，剩余{self.PointName} {number}',)).start()
                self.base.WritKillWarncfg(steamid)
                self.base.updatePoints(steamid,number)
            else:
                threading.Thread(target=rcon.send, args=(f'AdminBroadcast {name} 兑换失败，{self.PointName}不足{self.KillWarnNumber}，请先签到获取',)).start()
        else:
            threading.Thread(target=rcon.send,args=(f'AdminBroadcast {name} 兑换失败，还未拥有{self.PointName}，请先签到获取',)).start()
                        ...
                        if player_id not in outboard_start_time:
                            ...
                            with lock:
                                ...
                                if idle_time > 10 * 60 :  # 10分钟 = 10 * 60 秒
                                    ...
                                    threading.Thread(target=send, args=(f'AdminKick {player_id} 空闲时间过长，自动踢出',)).start()
                    else:
                        # 玩家已经加入小队
                        with lock:
    ...
    def squadcreate_Broadcast(self,chat_json,rcon):
        ...
        if message:
            ...
            threading.Thread(target=rcon.send,args=(f'AdminWarn {steamid} 你还未拥有小队！请先创建队伍后再查询建队信息！',)).start()
    ...
    def kill_death_warn(self,chat_json,rcon):
        ...
        if data:
            ...
            wound = data['wound']
            death = data['death']
            threading.Thread(target=rcon.send,args=(f'AdminWarn {steamid} 击倒:{wound}人 击杀:{kill}人 死亡{death}次',)).start()
        else:
    ...
    def extract_op(self,rcon):
            ...
            for player in players:
                ...
                for admin_player in admin_list:
                    if steamid == admin_player['steamid'] and admin_player['power'] in power:
                        ...
                        threading.Thread(target=rcon.send, args=(f'AdminWarn {steamid} 有玩家正在呼叫OP，请停止游戏，尽快处理！',)).start()
                        break
        ...
        threading.Thread(target=rcon.send,args=(f'AdminBroadcast 当前共计{len(admin_info)}位OP在线\n{admin_info_combined}',)).start()
...
def ServerMessageHandle(rcon):
    ...
    while True:
        ...
        if 'has possessed' in decoded_data:
            ...
            rcon.send(f'AdminWarn {name} 您已飞天，执法过程中建议录屏留存证据')
        if 'unpossessed' in decoded_data:
            ...
            if name in flight_start_times:
                ...
                if flight_duration < 60:
                    rcon.send(f'AdminWarn {name} 您的飞天已结束，本次飞天时长：{flight_duration:.0f} 秒')
                else:
...
def AutoClearPlayer(send):
    server_id = Readyaml('serverId')
    mod = moudel()
    match = re_str()
    while True:
        body = requests.get(f'http://150.138.84.157:5004/get-server-data/{server_id}').json()['serverData']
        playerlist = match.plList(body)
        threading.Thread(target=mod.TeamPlayerCountStat, args=(playerlist,)).start()
        threading.Thread(target=mod.ClearPlayer,args=(playerlist,send)).start()
...
def squad_rcon():
    ...
    threading.Thread(target=AutoClearPlayer,args=(rcon.send,)).start()
